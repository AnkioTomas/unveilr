...
const { isWin, rollbackLogger } = require('./utils')
...
function save(name, content) {
  ...
  mkdirs(path.dirname(name), () =>
    ioLimit.runWithCb(fs.writeFile.bind(fs), name, content, err => {
      if (err) {
        if (isWin()) {
          logger.debug('Save file error: ' + err)
        } else {
...
function commandExecute(cb, helper, argv) {
  logger.time('Total use')

  function endTime() {
    ioEvent.check(() => logger.timeEnd('Total use'))
  }
  argv = argv || process.argv
  let orders = []
  for (let order of argv) if (order.startsWith('-')) orders.push(order.slice(1))
  let iter = argv[Symbol.iterator](),
    nxt = iter.next(),
    called = false,
