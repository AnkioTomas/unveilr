...
const parserZArray = require('./parserZArray')
const saveMapper = {}
...
function doWxml(state, dir, name, code, z, xPool, rDs, wxsList, moreInfo) {
  ...
  let r = { son: [] }
  analyze(esprima.parseScript(code).body, z, { [rname]: r }, xPool, {
    [rname]: r,
  })
  let ans = []
...
module.exports = async function parserWxml(code, dir) {
  const wxsList = {}
  ...
  for (let info in rF)
    if (typeof rF[info] == 'function') {
      let name = path.resolve(dir, (info[0] == '/' ? '.' : '') + info),
        ref = rF[info]()
      pF[ref] = info
      saveMapper[name] = doWxs(requireInfo[ref].toString(), info)
    }
  for (let info in rF)
  ...
  for (let name in rE) tryWxml(dir, name, rE[name].f.toString(), z, x, rD[name], wxsList, void 0)
